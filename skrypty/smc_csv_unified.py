...
class LOSSMCAdaptiveController(Node):
    ...
    def _log(self, t, dist, yaw_ref, yaw_err, base, turn, left, right):
        ...
        J_roll = 14.0 * self.ind.I_roll
        J_pitch = 2.0 * self.ind.I_pitch
        J_roll_rate = 2.5 * self.ind.I_roll_rate
        J_pitch_rate = 0.5 * self.ind.I_pitch_rate
        J_control = 0.20 * self.ind.I_control
        J_no_progress = 4.0 * self.ind.I_no_progress
        J_goal_heading = 0.6 * self.ind.I_heading

        speed_ref = float('nan')
        J_inst = float('nan')

        self._log_file.write(
            f"{t:.3f},3,{state_code},{self.wp_idx + 1},"
            f"{self.x:.3f},{self.y:.3f},{dist:.3f},"
            f"{math.degrees(self.yaw):.3f},{math.degrees(yaw_to_wp):.3f},"
            f"{math.degrees(yaw_ref):.3f},{math.degrees(yaw_err):.3f},{math.degrees(self.yaw_rate):.3f},"
            f"{speed_ref:.6f},{self.speed:.6f},"
            f"{math.degrees(self.roll):.6f},{math.degrees(self.roll_rate):.6f},"
            f"{math.degrees(self.roll_window.rms()):.6f},{math.degrees(self.ind.roll_rms_total()):.6f},"
            f"{math.degrees(self.pitch):.6f},{math.degrees(self.pitch_rate):.6f},"
            f"{math.degrees(self.pitch_window.rms()):.6f},{math.degrees(self.ind.pitch_rms_total()):.6f},"
            f"{base:.6f},{turn:.6f},{left:.6f},{right:.6f},"
            f"{J_inst:.6f},0.000000,0.000000,0.000000,0.000000,"
            f"0.000000,0.000000,0.000000,0.000000,0.000000,0.000000,0.000000,0.000000,"
            f"{self.ind.J_total:.8f},{J_roll:.8f},{J_pitch:.8f},{J_roll_rate:.8f},{J_pitch_rate:.8f},"
            f"{J_goal_heading:.8f},{J_no_progress:.8f},{J_control:.8f},"
            f"{self.ind.I_roll:.8f},{self.ind.I_pitch:.8f},{self.ind.I_roll_rate:.8f},{self.ind.I_pitch_rate:.8f},"
            f"{self.ind.I_control:.8f},{self.ind.I_no_progress:.8f},{self.ind.I_heading:.8f}\n"
        )

...
class SamplingMPCStableAutopilot(Node):
    ...
    W_MPC_PROGRESS = 30.00
    W_MPC_HEADING = 42.00
    W_MPC_WRONG_WAY = 85.00

    W_MPC_WAVE_ROLL = 1.15
    W_MPC_PITCH = 0.80
    W_MPC_CONTROL = 0.030
    W_MPC_SLEW = 0.080
    W_MPC_TURN = 0.040
    ...
    def _candidate_sets(self, tx, ty):
        ...
        if dist_now < 3.0 * self.WAYPOINT_RADIUS:
            base_candidates = [-18.0, -10.0, 0.0, 6.0, 12.0, 20.0, 30.0, actual_base]
            diff_candidates = [-65.0, -45.0, -25.0, 0.0, 25.0, 45.0, 65.0, actual_diff]
        ...
        if roll_severity > 1.30:
            base_candidates = [-12.0, 0.0, 6.0, 14.0, 24.0, 34.0, actual_base]
            diff_candidates += [-85.0, -65.0, -45.0, 0.0, 45.0, 65.0, 85.0]
    ...
    def _tail_policy(self, x, y, yaw, v, yaw_rate, tx, ty, prev_base, prev_diff):
        ...
        if abs_err > math.radians(115.0):
            ...
            speed_ref = 0.25
        elif dist < 2.0 * self.WAYPOINT_RADIUS:
            speed_ref = 0.35
        elif dist < 10.0:
            speed_ref = self.APPROACH_SPEED
        elif dist < 25.0:
            speed_ref = 1.05
        else:
        ...
        if (abs_err > math.radians(100.0) and v > 0.45) or (dist < 8.0 and v > 0.95):
            desired_base = min(desired_base, -10.0)
    ...
    def _simulate_rollout(self, first_base, first_diff, tx, ty, collect_path=False):
        ...
        for k in range(self.MPC_HORIZON):
            ...
            if progress < 0.0:
                J_progress += 10.0 * (-progress / 1.0) ** 2 + 2.5
            else:
    ...
    def _print_status(self, now, dist, yaw_to_wp, yaw_err_to_wp, base_cmd, diff_cmd, info):
        ...
        print('\033[H\033[J', end='')
        print('════════════════════════════════════════════════════════════')
        print('  Sampling MPC FIXED WP3 – WAM-V bez dokładnego modelu hydrodynamicznego')
        print('════════════════════════════════════════════════════════════')
    ...
    def _setup_plot(self):
        ...
        self.ax.set_title('Sampling MPC FIXED WP3: trajektoria + okręgi waypointów')

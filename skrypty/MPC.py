...
class SamplingMPCStableAutopilot(Node):
    ...
    def __init__(self):
        ...
        self._log_file.write(
            't,state,wp_idx,x,y,dist,'
            'yaw_deg,yaw_to_wp_deg,yaw_err_to_wp_deg,'
            'speed_meas,roll_deg,roll_rms_window_deg,roll_rms_total_deg,'
            'pitch_deg,pitch_rms_window_deg,pitch_rms_total_deg,'
            'base_cmd,diff_cmd,tl,tr,'
            'J_mpc,J_mpc_distance,J_mpc_final,J_mpc_progress,J_mpc_heading,J_mpc_wrong_way,J_mpc_wave_roll,'
            'J_mpc_pitch,J_mpc_control,J_mpc_slew,J_mpc_turn,'
            'J_total,J_roll,J_pitch,J_roll_rate,J_pitch_rate,J_goal_heading,J_no_progress,J_control,'
            'I_roll,I_pitch,I_roll_rate,I_pitch_rate,I_control\n'
        )
    ...
    def _log(self, now, dist, yaw_to_wp, yaw_err_to_wp, info):
        ...
        self._log_file.write(
            f'{t:.2f},{self.state},{self.wp_idx + 1},'
            f'{self.x:.2f},{self.y:.2f},{dist:.2f},'
            f'{math.degrees(self.yaw):.2f},{math.degrees(yaw_to_wp):.2f},{math.degrees(yaw_err_to_wp):.2f},'
            f'{self.speed_meas:.3f},'
            f'{math.degrees(self.roll):.3f},{math.degrees(self.roll_rms.get()):.3f},{math.degrees(self.ind.rms_roll()):.3f},'
            f'{math.degrees(self.pitch):.3f},{math.degrees(self.pitch_rms.get()):.3f},{math.degrees(self.ind.rms_pitch()):.3f},'
            f'{self._last_base:.2f},{self._last_diff:.2f},{self._last_tl:.2f},{self._last_tr:.2f},'
            f'{info["J"]:.5f},{info["distance"]:.5f},{info["final"]:.5f},{info["progress"]:.5f},{info["heading"]:.5f},'
            f'{info["wave_roll"]:.5f},{info["pitch"]:.5f},{info["control"]:.5f},'
            f'{info["slew"]:.5f},{info["turn"]:.5f},'
            f'{self.ind.J_total:.6f},{self.ind.J_roll:.6f},{self.ind.J_pitch:.6f},'
            f'{self.ind.J_roll_rate:.6f},{self.ind.J_pitch_rate:.6f},'
            f'{self.ind.J_goal_heading:.6f},{self.ind.J_no_progress:.6f},{self.ind.J_control:.6f},'
            f'{self.ind.I_roll:.6f},{self.ind.I_pitch:.6f},'
            f'{self.ind.I_roll_rate:.6f},{self.ind.I_pitch_rate:.6f},{self.ind.I_control:.6f}\n'
        )
